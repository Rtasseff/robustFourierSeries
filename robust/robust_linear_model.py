...
"""
import numpy as np
from tools.tools import rank
from linear_model import WLS, GLS
import norms
import scale
from model import (LikelihoodModel,
        LikelihoodModelResults)
from tools.decorators import (cache_readonly,
        resettable_cache)
    ...
    **Attributes**
    ...
    def fit(self, maxiter=50, tol=1e-8, scale_est='mad', init=None, cov='H1',
        ...
        while (np.all(np.fabs(criterion[self.iteration]-\
                ...
                (self.iteration < maxiter) and (self.scale > 1E-19)):
#            self.weights = self.M.weights((self.endog - \
#                    wls_results.fittedvalues)/self.scale)
            self.weights = self.M.weights(wls_results.resid/self.scale)
            try:
		wls_results = WLS(self.endog, self.exog,weights=self.weights).fit()
            except:
		wls_results = WLS(self.endog, self.exog,weights=self.weights).fit(method="qr")
